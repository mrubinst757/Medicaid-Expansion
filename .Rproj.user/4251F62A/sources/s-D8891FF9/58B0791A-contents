...
point_estimate_plot <- function(data_subset, file_extension) {
  data_subset %>%
    ...
    xlab("Weight Set") +
    scale_fill_brewer(palette = "Dark2", name = "Adjustment") +
...
republican_contrast_diff_plot <- function(data_subset_c1, data_subset_c2, file_extension) {
  ...
  palette <- brewer_pal(palette = "Dark2")(6)[5:6]
  ...
  plot_data(data_subset_c1, "Primary Dataset") %>%
    ...
    geom_bar(stat = "identity", position = "dodge") +
    scale_fill_brewer(palette = "Accent", name = "Dataset") +
    theme_minimal() +
...
produce_heatmaps <- function(txfx_results, file_extension) {
  ...
  loo_heatmap(c1_diff_state, c("sigma_uu_i_modeled"), "proc", othr[1])
  loo_heatmap(c1_diff_proc, c("sigma_uu_i_modeled"), "data", othr[2])
  loo_heatmap(c2_diff_state, c("sigma_uu_i_modeled"), "proc", othr[3])
  loo_heatmap(c2_diff_proc, c("sigma_uu_i_modeled"),"data", othr[4])
}
...
rdiff_distr <- function(loo_table, original) {
  ...
  original <- original %>%
    mutate_at("Sigma estimate", ~stringr::str_replace_all(.,
