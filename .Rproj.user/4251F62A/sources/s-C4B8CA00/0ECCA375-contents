...
generate_sbw_weights <- function(X_1, tols, targets, sigma2.y = 1, re = 0,
  ...
  calc_sbw_weights <- partial(optweight.svy, formula = balancing_formula, std.cont = FALSE, std.binary = FALSE,
...
generate_synth_weights <- function(X_1, targets) {
  X0 <- as.matrix(t(X_1[variables]))
  X1 <- as.matrix(targets)
  variables <- names(targets)
  custom.v <- rep(1/length(variables), length(variables))
  if (any(grepl("hins_unins", variables))) {
    custom.v[grep("hins_unins", variables)] <- 20*custom.v[grep("hins_unins", variables)]
    custom.v <- custom.v/sum(custom.v)
  }
  
  results <- try(synth(X0 = X0, X1 = X1, Z0 = X0, Z1 = X1, 
                scale = FALSE, 
                lambda = 0.001, 
                custom.v = custom.v))
  
  if (class(results) == "try-error") {
    weights <- rep(1, ncol(X0))
  }
  
  if (class(results) != "try-error") {
    weights <- results$solution.w
  }
...
# calculate overlap weights using logistic regression
generate_overlap_weights <- function(data, variables) {
  formula <- as.formula(paste0("treatment ~ ", paste0(variables, collapse = "+")))
  
  model <- glm(formula, data, family = binomial(link = "logit"))
  
  predictions <- round(predict(model, type = "response"), 2)
  status <- if_else(all(predictions %in% c(0, 1)), "failed", "converged")
  ...
  data <- data %>%
    mutate(pi = predict(model, type = "response"),
           weights = if_else(treatment == 1, 1 - pi, pi))
  
  txfx <- data %>%
    group_by(treatment) %>%
    dplyr::summarize(hins_unins_pct_2014 = sum(hins_unins_pct_2014*weights)/sum(weights)) %>%
    mutate(txfx = hins_unins_pct_2014[treatment == 1] - hins_unins_pct_2014[treatment == 0])
  
  list(txfx = unique(txfx$txfx), weights = data$weights, status = status)
}

# iterate data list 
iterate_data_oate <- function(data_list, variables) {
  oate_results <- map(data_list, ~generate_overlap_weights(.x, variables))
  names(oate_results) <- names(data_list)
  oate_results
}

# iterate overlap weights across variable groups
iterate_covariates_oate <- function(data_list, variable_list) {
  oate_results <- map(variable_list, ~iterate_data_oate(data_list, .x))
  names(oate_results) <- names(variable_list)
  oate_results
}
