...
years <- stringr::str_trunc(c("2010", "2011", "2012", "2013", "2014"), 2, side = 'left', ellipsis = '') 

urls <- sprintf('https://www2.census.gov/programs-surveys/acs/data/pums/20%s/', years)

household.files      <- sprintf('csv_h%s', tolower(state.abb))
household.files.src  <- unlist(map(urls, ~paste0(.x, household.files, '.zip')))
household.files.dest <- unlist(map(c(2010:2014), ~paste0(household.files, .x, '.zip')))

person.files        <- sprintf('csv_p%s', tolower(state.abb))
person.files.src    <- unlist(map(urls, ~paste0(.x, person.files, '.zip')))
person.files.dest   <- unlist(map(c(2010:2014), ~paste0(person.files, .x, '.zip')))

map2(household.files.src, household.files.dest, ~download(.x, .y))
